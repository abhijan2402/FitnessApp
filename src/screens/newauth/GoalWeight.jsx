...
const GoalWeight = ({ navigation }) => {
    ...
    return (
        <ScrollView style={{ backgroundColor: "white", height: "100%" }}>
            <SlideHeader />
            <LiftMan width={124} height={189} style={{ alignSelf: "center", marginVertical: 20 }} />
            <View style={styles.ProfileView}>
                ...
                <View style={styles.InnerMain}>
                    <TouchableOpacity onPress={() => { setlbs(true); setkg(false) }} style={[styles.InnerMainView, { backgroundColor: lbs ? "#8C80F8" : "white" }]}>
                        <Text style={{ fontSize: 13, color: lbs ? "white" : "black", fontWeight: "500" }}>lbs</Text>
                    </TouchableOpacity>
                    <TouchableOpacity onPress={() => { setlbs(false); setkg(true) }} style={[styles.InnerMainView, { backgroundColor: kg ? "#8C80F8" : "white" }]}>
                        <Text style={{ fontSize: 13, color: kg ? "white" : "black", fontWeight: "500" }}>kg</Text>
                    </TouchableOpacity>
                </View>
                <Input
                    placeholder={'Weight'}
                ...
                <NewButtob title={"Continue"} onPress={() => { navigation.navigate(SCREENS.HEIGHT) }} />
            </View>
        </ScrollView>
    )
}
